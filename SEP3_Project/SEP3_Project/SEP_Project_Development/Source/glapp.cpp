...
GLApp::GLModel GLApp::mystery_model() {
	...
	GLfloat octagonRadius = 0.4f;
	GLfloat octagonRadius2 = 0.3f;
	GLfloat octagonVertices = octagonRadius / static_cast<float>(sqrt(2));
	GLfloat octagonVertices2 = octagonRadius2 / static_cast<float>(sqrt(2));

	std::vector<glm::vec2> pos_vtx
	{
		//Vertex for Box
		//0,1,2,3
		glm::vec2(1.0f, 0.2f), glm::vec2(0.2f, 0.2f),
		glm::vec2(1.0f, -0.2f), glm::vec2(0.2f, -0.2f),
		...
		//9 10 11 12
		glm::vec2(0.0f, octagonRadius), glm::vec2(octagonVertices, octagonVertices),
		glm::vec2(octagonRadius, 0.0f), glm::vec2(octagonVertices, -octagonVertices),
		...
		//17 18 19 20
		glm::vec2(0.0f, octagonRadius2), glm::vec2(octagonVertices2, octagonVertices2),
		glm::vec2(octagonRadius2, 0.0f), glm::vec2(octagonVertices2, -octagonVertices2),
		...
		//26,27,28,29
		//Hour Hand
		glm::vec2(0.05f, 0.175f), glm::vec2(0.05, 0.0f),
		glm::vec2(-0.05,0.175f) ,glm::vec2(-0.05, 0.0f)	,

		//30 31 32 33
		//Minute Hand
		glm::vec2(0.0f, 0.02f), glm::vec2(0.0f,-0.02f),
		glm::vec2(-0.25f, 0.02f) ,glm::vec2(-0.25f, -0.02f)
	};
	...
	// Set a container for the color vertex
	std::vector<glm::vec3> clr_vtx;



	//*****************************************************************************************************
	// Setting the colors for each of the vertexes. (Triangles)
	for (size_t i{}; i < 8; i++)
	{
		float red = 210.0f / 256.0f;
		float green = 180.0f / 256.0f;
		float blue = 140.0f / 256.0f;
	...
	{
		float red = 20.0f / 256.0f;
		float green = 20.0f / 256.0f;
		float blue = 20.0f / 256.0f;
	...
	for (size_t i{ 26 }; i < pos_vtx.size(); i++)
	{
		float red = 0;
		float green = 0;
		float blue = 0;
	...
	std::array<GLushort, 72> idx_vtx{
		0, 1, 2,  // 1st triangle with counterclockwise winding is specified by
		// vertices in VBOs with indices 0,1,2
		2, 3, 1,	  // 2nd trinagle with counter clockwise winding 


		4, 5, 6,  // 1st triangle with counterclockwise winding is specified by
		// vertices in VBOs with indices 0,1,2
		6, 5, 7,	  // 2nd trinagle with counter clockwise winding 

		// Octagon vertices (bigger)
		8, 9, 10,
		8, 10, 11,
		8, 11, 12,
		8, 12, 13,
	...
	// Declare a variable for idx element count
	GLuint idx_elem_cnt;
	// Set the number of indices in the idx_vtx array to the idx element count variable
	idx_elem_cnt = idx_vtx.size();

	// Declar an element buffer object and fill it with data from idx_vtx
	GLuint ebo_hdl;
	...
	// Attach the element buffer object to the vertex array object
	glVertexArrayElementBuffer(vaoid, ebo_hdl);

	// Unbind the vertex array object
	glBindVertexArray(0);

	// Set the model properties based on the above calculations

	mdl.vaoid = vaoid;								// Model Vao ID
	mdl.primitive_type = GL_TRIANGLES;				// Model Primitive Type
	mdl.draw_cnt = idx_vtx.size();					// Model Draw Count
	mdl.primitive_cnt = pos_vtx.size();				// Model Primitive Count
